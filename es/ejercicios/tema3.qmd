...
Retorna `True` si el entero positivo `n` es un número **perfecto** y `False` en caso contrario. Un número es perfecto si es igual a la suma de sus divisores propios (divisores distintos de él mismo). Por ejemplo, $6 = 1 + 2 + 3$ por lo que `es_perfecto(6)` devuelve `True`.

:::

::: {.callout-note title="1.2 Función `siguiente_primo(n)`:" collapse="true"}

Devuelve el menor primo estrictamente mayor que `n`. Reutiliza `es_primo` del módulo `numeros.py`. Por ejemplo, `siguiente_primo(7)` devuelve 11.
...
::: {.callout-note title="Algoritmo para 3.4.3 y 3.4.4" collapse="true"}
...
La función `biseccion(f, a, b, tol)` implementa el **método de bisección** para encontrar una raíz de la función continua `f` en el intervalo `(a, b)` con una tolerancia `tol` con valor por defecto de `1e-6`. La función debe verificar que `f(a)` y `f(b)` tengan signos opuestos antes de iniciar el proceso. Si no se cumple esta condición, la función debe retornar `None`.
