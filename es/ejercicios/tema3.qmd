...
Calcula la raíz cúbica de un número real `x` utilizando el **método de Newton-Raphson** con una tolerancia `tol` con valor por defecto de `1e-10`. Admite `x` negativo. La iteración es 
$$
...
- Si $f(a)$ y $f(c)$ tienen **signos opuestos**, la raíz está en la primera mitad. El nuevo intervalo será $[a, c]$ (es decir, $b$ toma el valor de $c$ `biseccion(f, a, c, tol)`).
- Si tienen el **mismo signo**, la raíz obligatoriamente está en la segunda mitad. El nuevo intervalo será $[c, b]$ (es decir, $a$ toma el valor de $c$ `biseccion(f, c, b, tol)`).

:::

**Ejemplo:**

Si `f(x) = x^3 - x - 2` en el intervalo `[1, 2]` la solución exacta es
$$
x=\frac{1}{3a}+a\text{ con }a=\left( \frac{\sqrt{702}}{27}+1\right) ^{1/3}
$$
por lo cual $x \approx 1.5213797068045676$.

**Pista:**

La implementación recursiva de este método es directa y elegante.
