...
Una vez creada la cuenta de GitHub y completado el perfil, debes **reportar tu cuenta de GitHub en el Aula Digital**. Esto permitirá agregar tu cuenta a la lista de estudiantes autorizados en la organización del curso en GitHub.

Para esto se ha generado una [**tarea en el Aula Digital**](https://ad.uib.es/estudis2627/mod/assign/view.php?id=318505). Debes:
1. Acceder al Aula Digital y localizar la tarea **"Reportar su cuenta de GitHub en el Aula Digital"**.
2. Introducir tu **nombre de usuario de GitHub** (el que elegiste al crear la cuenta) en el campo de entrega de la tarea.
...
4. Familiarizarte con la interfaz de **VS Code** (el **IDE** a utilizar) y la terminal integrada.
5. Guardar cambios en tu repositorio usando el **Sistema de Control de Versiones Git** .

::: {#imp-codespace .callout-important title="IDE"}
Un **IDE** (Integrated Development Environment, por sus siglas en inglés) es una aplicación de software que concentra todas las herramientas que un programador necesita para crear programas en una única interfaz. En su forma más básica, un IDE combina tres herramientas fundamentales en una misma ventana para que no tengas que cambiar de programa constantemente:

1. **Un editor de código inteligente:** Un procesador de texto diseñado específicamente para programar. Colorea las palabras clave, autocompleta el código y te avisa de errores tipográficos en tiempo real.
2. **Un motor de ejecución (Compilador/Intérprete):** Te permite ejecutar y probar tu programa directamente haciendo clic en un botón (como el botón "Play"), traduciendo tu código al lenguaje que entiende la máquina.
3. **Un depurador (Debugger):** Una herramienta de diagnóstico que te permite "pausar" el programa mientras está funcionando, analizarlo paso a paso y encontrar dónde te has equivocado en la lógica.

En resumen, un IDE (como Visual Studio Code) existe para eliminar el trabajo mecánico y repetitivo, permitiendo al usuario centrarse puramente en resolver el problema abstracto. Actualmente escribir programas sin un IDE es como confeccionar un mueble solo con un martillo y clavoss.
:::
...
::: {#imp-codespace .callout-important title="Sistema de Control de Versiones (Git)"}
Un **Sistema de Control de Versiones** es una herramienta que elimina el caos de trabajar con múltiples versiones de un mismo archivo. Actúa como una máquina del tiempo fotográfica para tu código. Sus tres ventajas principales son:

- **Historial absoluto:** Registra qué línea de código se cambió, cuándo se cambió y quién lo hizo (acompañado de una nota explicativa).
- **Viaje en el tiempo:** Si hoy escribes algo que rompe el programa que ayer funcionaba perfectamente, puedes presionar un "deshacer mágico" y volver a la fotografía exacta de ayer.
- **Colaboración coordinada:** Permite que varios usuarios trabajen en el mismo archivo al mismo tiempo. El sistema es lo bastante inteligente para fusionar el trabajo de todos sin borrar lo del compañero.
:::

::: {.callout-note title="8 - Guardar los cambios en tu repositorio" collapse="true"}

Para guardar los cambios de forma permanente en tu repositorio `t01-fundamentos` en GitHub, usamos el sistema de control de versiones llamado **Git** que ya está instalado en tu Codespace. Este proceso consta de tres pasos obligatorios que siguen la **"Analogía del Envío por Correo"**:

1. **Stage (Preparar):** Meter el archivo en la caja de cartón.
2. **Commit (Confirmar):** Cerrar la caja y pegarle una etiqueta que dice qué hay dentro.
3. **Push (Empujar):** Entregar la caja al cartero para que la lleve al servidor de GitHub. 

A continuación las instrucciones precisas para cumplir con estos pasos de dos maneras diferentes: usando la **interfaz gráfica de VS Code** o usando la **terminal**.

::: {.callout-tip appearance="simple" title="Variante 1: Usando la Interfaz Visual (Source Control)"}
1. **Abre el panel de Source Control:** (el tercer icono en la barra lateral izquierda parece un pequeño grafo o rama). Al hacer clic, verás tu archivo `prueba.py` listado bajo la sección **Changes** (Cambios).

2. **Mete el archivo en la caja (Stage Changes):** Pasa el ratón por encima del nombre `prueba.py`. Verás que aparece un icono con el símbolo más (**`+`**). Haz clic en él. Tu archivo pasará a una nueva lista llamada **Staged Changes** (Cambios preparados).

3. **Pon la etiqueta y cierra la caja (Commit):** Justo encima de la lista de archivos hay un cuadro de texto que dice *Message*. Escribe ahí un resumen claro de lo que has hecho, por ejemplo: `Añadido programa de prueba que funciona correctamente` (si Copilot está activado, puede sugerirte un mensaje automáticamente haciendo clic en las estrellas al final de *Message* (Generate Commit Message)). A continuación, haz clic en el botón azul que dice **Commit**.

4. **Envía el paquete a GitHub (Push / Sync):** El botón azul cambiará y ahora dirá **Sync Changes** (Sincronizar cambios). Haz clic en ese botón azul. En unos segundos, tu código estará seguro y guardado en tu repositorio `t01-fundamentos`.
:::

::: {.callout-tip appearance="simple" title="Variante 2: Usando Comandos (la `Terminal`)"}
1. **Abre la `Terminal` integrada:** Si no tienes la terminal (consola) abierta en la parte inferior de tu Codespace, ve al menú hamburguesa en la parte superior de la pantalla (las tres líneas horizontales), has clic y selecciona **Terminal > New Terminal**.
...
2. Escribe el siguiente comando y pulsa **Enter**:
    ...
    El sistema te responderá en rojo que `prueba.py` ha sido modificado o es un archivo nuevo, indicando que aún no está guardado en el repositorio.

3. **Mete el archivo en la caja (Stage Changes):** Escribe el siguiente comando y pulsa **Enter**:

    ```bash
    git add prueba.py
    ```

4. **Pon la etiqueta y cierra la caja (Commit):** Escribe el siguiente comando (las comillas son obligatorias) y pulsa **Enter**:

    ```bash
    git commit -m "Añadido programa de prueba que funciona correctamente"
    ```

5. **Envía el paquete a GitHub (Push):** Escribe el siguiente comando y pulsa **Enter**:
...
Cualquiera de los dos métodos logra exactamente lo mismo. Recomiendo empezar por la variante 1 para entender el flujo conceptual (Stage -> Commit -> Push) visualmente, y luego transicionar a la Variante 2 en prácticas posteriores.

:::

::: {.callout-note title="9 - Cerrar el Codespace" collapse="true"}
