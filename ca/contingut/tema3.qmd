...
1. **Capçalera (o signature)**: `def nombre(parámetros):` — defineix el nom i els paràmetres d'entrada. Inclou els elements següents:
...
::: {#imp-parametros .callout-important title="Paràmetres i arguments d'una funció"}
...
print(potencia(base=2, exponente=10))   # per paraula clau: equivalent
print(potencia(exponente=10, base=2))   # l'ordre ja no importa
```
...
::: {#imp-keyword-only .callout-important title="Paràmetres exclusivament de paraula clau"}
...
raiz(2, tol=1e-6)    # correcte
# raiz(2, 1e-6)      # TypeError: tol és keyword-only
```
...
::: {#tip-generador .callout-tip title="Funció generadora"}
...
::: {#nte-fibonacci-gen .callout-note title="Generador de la successió de Fibonacci"}
...
::: {#nte-primos-gen .callout-note title="Generador de nombres primers"}
...
::: {#cau-generador-agotado .callout-caution title="Un generador només es pot recórrer una vegada"}
...
Les variables definides fora de qualsevol funció tenen **àmbit global** i són visibles des de qualsevol punt del mòdul, inclòs el cos de les funcions. En Python, un **mòdul** és simplement un fitxer `.py`. Quan escrius codi en `programa.py` i l'executes, aquell fitxer és el teu mòdul. Els mòduls es tracten en detall a la secció 3.5.
...
Els `assert` comproven que el programador usa la funció correctament; els `raise` gestionen entrades incorrectes externes. El mecanisme `raise`/`except` s'estudia a la secció 3.4.
:::
...
A la secció anterior es va definir el contracte d'una funció: precondicions que el cridador ha de complir i postcondicions que la funció garanteix. Però el contracte només descriu el camí feliç. Què passa quan alguna cosa falla? Python usa les **excepcions** com a mecanisme estàndard per senyalar errors en temps d'execució i comunicar-los de forma controlada.

::: {#tip-excepcion .callout-tip title="Excepció"}
Una **excepció** és un objecte que Python crea quan detecta un error durant l'execució. En llançar-se, interromp el flux normal del programa i ascendeix per la pila de crides fins que algun codi la capturi i la gestioni. Si cap codi la captura, el programa acaba i Python mostra un missatge d'error (*traceback*).
:::

Les excepcions més freqüents en codi matemàtic són:

| Excepció | Es llança quan… | Exemple |
|---|---|---|
| `ValueError` | El tipus és correcte però el valor no té sentit | `math.sqrt(-1)` |
| `TypeError` | L'operació no pot aplicar-se al tipus rebut | `"tres" + 3` |
| `ZeroDivisionError` | Divisió o mòdul per zero | `1 / 0` |
| `IndexError` | S'accedeix a una posició fora del rang de la llista | `v[10]` amb `len(v) == 3` |
| `KeyError` | S'accedeix a una clau que no existeix en un diccionari | `d["z"]` si `"z"` no és a `d` |
| `RecursionError` | La recursió supera el límit de profunditat de Python | Sense cas base |

Aquestes excepcions no són errors de Python, sinó senyals ben definides que el propi llenguatge o el teu codi pot llançar i capturar.

### **3.4.1 Llançar excepcions amb `raise`**

La instrucció `raise` llança una excepció des del teu propi codi. És l'eina correcta per rebutjar entrades invàlides quan la funció no pot continuar:

```python
raise TipusExcepció("missatge descriptiu")
```

El tipus d'excepció s'ha de triar en funció de la naturalesa del problema: `ValueError` per a valors fora de domini, `TypeError` per a tipus incorrectes. El missatge ha de ser informatiu.

Reprenent la funció `raiz_cuadrada` de la secció anterior, la versió amb `raise` en lloc d'`assert` és adequada quan la funció forma part d'un programa que rep dades externes:
...
Ambdues instruccions poden verificar precondicions, però tenen propòsits distints:

| | `assert condicio` | `raise ValueError(...)` |
|---|---|---|
| **Propòsit** | Detectar errors del programador | Rebutjar entrades externes invàlides |
| **Es desactiva amb** | `python -O` | Mai |
| **Quan usar** | Durant el desenvolupament, en proves | En codi de producció, dades de l'usuari |
| **Missatge** | Opcional | Sempre informatiu |

La regla pràctica: usa `assert` per verificar suposicions sobre el *teu propi codi*; usa `raise` per rebutjar valors que arriben de l'*exterior* (usuari, fitxer, xarxa).
:::

### **3.4.2 Capturar excepcions amb `try/except`**

Llançar una excepció senyala que alguna cosa ha anat malament. Capturar-la permet que el programa reaccioni de forma controlada en lloc d'acabar abruptament. L'estructura és:

```python
try:
    # codi que pot llançar una excepció
except TipusExcepció as e:
    # codi que s'executa si ocorre l'excepció
```

El bloc `try` s'executa normalment fins que ocorre una excepció del tipus indicat. Llavors l'execució salta al bloc `except`, on `e` és l'objecte excepció i `str(e)` conté el missatge.

Un cas freqüent és convertir text a nombre — patró habitual en processar dades externes o entrada de l'usuari:
...
Es poden capturar diversos tipus d'excepció en blocs separats per respondre de forma diferent a cada error:
...
::: {#cau-bare-except .callout-caution title="No usar `except:` sense tipus"}
La forma `except:` sense especificar tipus captura **totes** les excepcions, incloent-hi `KeyboardInterrupt` (Ctrl+C) i `SystemExit`. Això pot deixar el programa atrapat en un estat inconsistent o impedir tancar-lo:

```python
# Malament: captura massa
try:
    resultado = calcular(x)
except:
    print("Algo salió mal.")

# Bé: captura només el que s'espera
try:
    resultado = calcular(x)
...
Com a regla general: captura només el tipus d'excepció que saps com gestionar.
:::

### **3.4.3 La clàusula `finally`**

El bloc `finally` conté codi que s'executa **sempre**, hi hagi o no excepció. L'ús més habitual és garantir que els recursos s'alliberen correctament:
...
try:
    ...
    print("Neteja: aquest bloc sempre s'executa.")
```

::: {#nte-finally .callout-note title="`finally` i gestió de recursos"}
En la pràctica, l'alliberament de recursos en Python es gestiona amb la instrucció `with`, que implementa `finally` de forma implícita i més segura. El patró `with` s'estudia al Tema 6 en treballar amb fitxers, que és on la seva utilitat es fa més evident.
:::

---

## 3.5. Concepte de mòdul. Importació i creació de mòduls propis
...
### **3.5.2 Creació de mòduls propis**
...
if __name__ == "__main__":
    ...
    # prova de precondició: factorial(-1) ha de llançar ValueError
    try:
        factorial(-1)
    except ValueError as e:
        print(f"Error: {e}")
...
### **3.5.3 El bloc `if __name__ == "__main__"`**
...
### **3.6.2 Exemple de mòduls del projecte `t03_ejemplo`**
...
if __name__ == "__main__":
    ...
    # prova de precondició: radi negatiu ha de llançar ValueError
    try:
        area_circulo(-1)
    except ValueError as e:
        print(f"Error: {e}")
    # prova de precondició: catet no positiu ha de llançar ValueError
    try:
        hipotenusa(-3, 4)
    except ValueError as e:
        print(f"Error: {e}")
...
if __name__ == "__main__":
    ...
    # prova de precondició: temperatura sota zero absolut ha de llançar ValueError
    try:
