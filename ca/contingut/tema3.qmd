...
```{pyodide}
def potencia(base, exponente):      # base i exponente són paràmetres
    return base ** exponente

print(potencia(2, 10))              # 2 i 10 són arguments → 1024
print(potencia(3, 3))               # 27
```

Els **paràmetres amb valor per defecte** permeten ometre alguns arguments en la crida quan el valor habitual és conegut d'antuvi:

```{pyodide}
def potencia(base, exponente=2):    # exponente té valor per defecte 2
    return base ** exponente

print(potencia(5))        # 25    (exponente pren el seu valor per defecte)
print(potencia(5, 3))     # 125   (se sobreescriu el valor per defecte)
...
# SyntaxError: paràmetres sense defecte no poden anar després dels que en tenen
def f(x=0, y):
    ...
```
...
Un exemple matemàtic que il·lustra bé els paràmetres amb valor per defecte és el **mètode de Newton-Raphson** per aproximar $\sqrt{x}$, que accepta el valor $x$ i una tolerància opcional:
...
print(raiz_cuadrada(2, tol=1e-4))    # 1.4142156862...  (menys precís)
```
...
La successió de Fibonacci també té una definició recursiva, però la seva implementació directa és molt ineficient:
...
`fibonacci(n)` amb la implementació anterior té cost exponencial $O(2^n)$: per calcular `fibonacci(40)` es realitzen més de mil milions de crides, perquè cada valor es recalcula repetidament des de zero. La versió iterativa del Tema 2 és infinitament més eficient. La recursió és una eina potent però no sempre eficient; triar entre recursió i iteració exigeix considerar el cost computacional.
...
La solució general a aquest problema s'anomena **memorització** (*memoization* en anglès): guardar el resultat de cada crida la primera vegada que es computa i retornar-lo directament en crides posteriors, sense repetir la feina. En temes posteriors es veurà com Python implementa memorització de forma transparent reduint el cost d'aquesta implementació de $O(2^n)$ a $O(n)$.
:::
...
def factorial_roto(n):
    return n * factorial_roto(n - 1)   # sense cas base
...
print(calcular())        # 42
# print(resultado)       # NameError: resultado no està definida fora de la funció
```
...
```{pyodide}
def cuadrado(x):
    y = x ** 2     # y és local a cuadrado
    return y

def cubo(x):
    y = x ** 3     # y és local a cubo, independent de l'anterior
    return y
...
```{pyodide}
PI = 3.141592653589793    # constant global (convenció: majúscules)

def area_circulo(r):
    return PI * r ** 2    # llegeix PI de l'àmbit global
...
def incrementar():
    global contador       # sense aquesta línia, Python crearia una variable local
    contador += 1
...
print(i)  # 4  — i segueix existint després del cicle
```
...
Una funció ben escrita no només calcula correctament: també comunica clarament què fa, què espera rebre i què garanteix retornar. Aquesta informació forma el **contracte** de la funció, i Python proporciona dos mecanismes complementaris per expressar-lo: els *docstrings* i les anotacions de tipus.
...
def es_primo(n):
    """
    Comprova si n és un nombre primer.

    Paràmetres
    ----------
    n : int
        L'enter a comprovar. Ha de ser major o igual que 2.

    Retorna
    -------
    bool
        True si n és primer, False en cas contrari.
    """
    if n < 2:
        return False
...
def potencia(base: float, exponente: int = 2) -> float:
    """Calcula base elevat a exponente."""
    return base ** exponente
...
L'exemple següent mostra les dues vies aplicades a la mateixa funció:
...
def raiz_cuadrada(x: float, tol: float = 1e-10) -> float:
    """
    Aproxima √x mitjançant el mètode de Newton-Raphson.

    Precondició:  x >= 0  i  tol > 0
    Postcondició: |resultado² - x| < tol * 100

    Paràmetres
    ----------
    x   : float  — El radicand. Ha de ser no negatiu.
    tol : float  — Tolerància de l'error. Ha de ser positiva.

    Retorna
    -------
    float — Aproximació de √x amb error inferior a tol.
    """
    assert x >= 0,   f"raiz_cuadrada: x ha de ser ≥ 0, rebut {x}"
    assert tol > 0,  f"raiz_cuadrada: tol ha de ser > 0, rebut {tol}"
...
# raiz_cuadrada(-1)             # AssertionError: x ha de ser ≥ 0, rebut -1
```
...
```python
if x < 0:
    raise ValueError(f"x ha de ser no negatiu, rebut {x}")
```
...
print(sqrt(2))        # sense prefix math.
print(pi)
print(factorial(10))
```
...
```python
# numeros.py
"""Mòdul amb funcions bàsiques de teoria de nombres."""

import math


def es_primo(n: int) -> bool:
    """Retorna True si n és primer (n ha de ser >= 2)."""
    if n < 2:
        return False
    for d in range(2, int(math.sqrt(n)) + 1):
        if n % d == 0:
...
def mcd(a: int, b: int) -> int:
    """Màxim comú divisor de a i b (algorisme d'Euclides)."""
    if b == 0:
        return a
    return mcd(b, a % b)
...
def mcm(a: int, b: int) -> int:
    """Mínim comú múltiple de a i b."""
    return abs(a * b) // mcd(a, b)


def factorial(n: int) -> int:
    """Factorial de n. Precondició: n >= 0."""
    assert n >= 0, f"factorial no està definit per a negatius, rebut {n}"
    if n == 0:
        return 1
...
if __name__ == "__main__":
    # Només s'executa en llançar: python numeros.py
    print("=== Proves del mòdul numeros ===")
...
```text
t03_ejemplo/
├── .gitignore             # Fitxers que Git no ha de rastrejar
├── pyproject.toml         # Configuració central del projecte i dependències
├── README.md              # Documentació principal
├── src/                   # Contenidor del codi font
│   └── t03_paquete/       # El teu paquet de Python real
│       ├── __init__.py    # Converteix la carpeta en paquet (pot estar buit)
│       ├── main.py        # Punt d'entrada del programa
│       ├── numeros.py     # Funcions de teoria de nombres
│       ├── geometria.py   # Funcions de geometria
│       └── conversor.py   # Funcions de conversió d'unitats
├── tests/                 # Proves automatitzades (separades del codi font)
│   ├── conftest.py        # Configuració i "fixtures" per a pytest
│   └── test_numeros.py    # Proves específiques per a numeros.py (es veuran al Tema 5)
└── docs/                  # Fitxers de documentació
...
```python
"""Mòdul de funcions geomètriques bàsiques."""

import math


def area_circulo(r: float) -> float:
    """Àrea d'un cercle de radi r. Precondició: r >= 0."""
    assert r >= 0, f"El radi ha de ser no negatiu, rebut {r}"
    return math.pi * r ** 2


def hipotenusa(a: float, b: float) -> float:
    """Longitud de la hipotenusa donats els catets a i b."""
    assert a > 0 and b > 0, "Els catets han de ser positius"
    return math.sqrt(a ** 2 + b ** 2)


def es_triangulo_valido(a: float, b: float, c: float) -> bool:
    """True si a, b, c satisfan la desigualtat triangular."""
    return a > 0 and b > 0 and c > 0 and a + b > c and a + c > b and b + c > a


if __name__ == "__main__":
    print(f"Àrea cercle r=1:    {area_circulo(1):.6f}")
    print(f"Hipotenusa 3, 4:     {hipotenusa(3, 4):.6f}")
    print(f"Triangle 3, 4, 5:   {es_triangulo_valido(3, 4, 5)}")
    print(f"Triangle 1, 2, 10:  {es_triangulo_valido(1, 2, 10)}")
```

**`conversor.py`**:

```python
"""Mòdul de conversió d'unitats."""
...
def celsius_a_kelvin(c: float) -> float:
    """Converteix graus Celsius a Kelvin. Precondició: c >= -273.15."""
    assert c >= -273.15, f"Temperatura per sota del zero absolut: {c}"
    return c + 273.15
...
**`main.py`** — punt d'entrada que coordina els mòduls:

```python
"""Eina matemàtica interactiva."""
...
def mostrar_menu() -> None:
    print("\n=== Eina matemàtica ===")
    print("  1. Test de primalitat")
    print("  2. MCD de dos enters")
    print("  3. Àrea d'un cercle")
    print("  4. Conversió de temperatura (°C → °F)")
    print("  0. Sortir")
    print("------------------------------")
...
def ejecutar() -> None:
    while True:
        mostrar_menu()
        opcion = input("Tria una opció: ")

        if opcion == "1":
            n = int(input("n: "))
            print(f"{n} és primer: {numeros.es_primo(n)}")
            ...
            r = float(input("Radi: "))
            print(f"Àrea = {geometria.area_circulo(r):.6f}")
...
`main.py` no conté cap càlcul propi: es limita a importar els mòduls i coordinar la interacció amb l'usuari. Aquesta separació entre **lògica de negoci** (els mòduls) i **punt d'entrada** (`main.py`) és una pràctica de disseny fonamental que escala bé a projectes de qualsevol mida.

El projecte complet `t03_ejemplo` està disponible al repositori del curs: [https://github.com/UIB-23200-Programacion/t03_ejemplo](https://github.com/UIB-23200-Programacion/t03_ejemplo){target="_blank"}. No cal que et concentris a memoritzar tots els detalls de l'estructura, ja que el projecte exemple té implementades les millors pràctiques de disseny en Python i el podràs reutilitzar en els teus propis repositoris. El que importa és comprendre el patró general i la filosofia de disseny que subjau. **El que és veritablement fonamental és entendre el codi en els mòduls `numeros.py`, `geometria.py`, `conversor.py` i en l'script `main.py` que s'han presentat anteriorment**.
:::
